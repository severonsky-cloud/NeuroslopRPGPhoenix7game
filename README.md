# Phoenix7 — текущий playable build v3L
...
- `v3l.html` — канонический вход в текущую линию v3L.
...
## Проверка живого мира

1. Запусти `00_START_HERE_CURRENT_BUILD_WINDOWS.bat`.
2. В браузере нажми `Ctrl+F5`.
3. Нажимай `F1`, пока не попадёшь на Красную дорогу или к Форту Заря.
4. Открывай журнал `J`.
5. Ищи караваны, патрули, бандитов, Жужжер и рыцарей.
6. Если караван атаковали, стреляй по налётчикам — они созданы как hostile monsters, чтобы текущее оружие работало по ним.
...
- Старые документы в `docs/` описывают предыдущие этапы разработки и не заменяют этот README для запуска v3L.
