...
## Дополнение Codex: v3M2B.2 «Повстанцы ночью»

Ветка `codex/v3m2b2-rebels-night` основана на `claude/v3-integration-build@36ed82f` и должна вливаться обратно в `claude/v3-integration-build`.

Добавлен четвёртый маршрут `rebels` со стадиями 60–69:

- код Ньен Ло и отдельный контакт `rebel_cell`;
- ожидание ночи у процедурного костра;
- ночной подход двух повстанцев к Посту Ришелье;
- рубильник, лампы и HUD внимания;
- чистое похищение либо 45-секундный бой с двумя волнами;
- checkpoint при смерти, refresh, уходе из зоны и гибели обоих союзников;
- исходы `rebels_clean` и `rebels_bloody`;
- persistent-награды с одноразовой гидратацией боеприпасов;
- флаг `dumont_abducted`: Дюмон исчезает, но не считается погибшим.

Ключевой новый файл: `src/game/v3/quests/taxRebelSystem.js`.

Полный технический отчёт и сценарий проверки: `docs/PHOENIX7_V3M2B2_REBELS_NIGHT_RU.md`.

После слияния следующими независимыми проходами остаются:

1. мобилизация крестьян (`mobilize`, 70–79);
2. фазовые призраки (`phase`, 80–89);
3. полноценный цикл глиняного каравана;
4. TTS и Quest Draft Builder — только отдельными задачами.
